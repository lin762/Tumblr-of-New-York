...
//  TumblrOfNewYork
//
//  Created by Chris lin on 1/12/18.
...
class PhotoViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        photoTableView.rowHeight = 250.0
    ...
    func updatePhotos(){
        let url = URL(string: "https://api.tumblr.com/v2/blog/humansofnewyork.tumblr.com/posts/photo?api_key=Q6vHoaVm5L1u2ZAW1fqv3Jw48gFzYVg9P0vH0VHl3GVy6quoGV" + "&offset=" + String(postCount))!
        let session = URLSession(configuration: .default, delegate: nil, delegateQueue: OperationQueue.main)
        session.configuration.requestCachePolicy = .reloadIgnoringLocalCacheData
        let task = session.dataTask(with: url) { (data, response, error) in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let sender = sender as? PhotoCell {
            //let cellIndex = photoTableView.indexPath(for: sender)
            let destinationViewController = segue.destination as! PhotoDetailsViewController
            destinationViewController.poster = sender.ImageView.image
        }
